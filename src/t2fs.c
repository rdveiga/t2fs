...
FileDescriptor* searchFile(char *name)
{
	...
	char fileName[41]; 	 //uma posição a mais para o \0
	int  firstRootBlock;
	int  cpyIdx;
	...
	for(blockIdx = 0; blockIdx < superblock.rootSize; blockIdx ++) //varre os blocos que compoem o diretório
	{
		...
		for(fileIdx = 0; fileIdx < numFiles; fileIdx ++) //varre todos os registros de arquivos de um bloco de diretório
		{
			...
			firstNameChar = fileReg[0];
			firstNameChar = firstNameChar & 128; //verifica se o bit 7 é "1"

			if(firstNameChar) //caso seja um arquivo válido, verifica o restante dos caracteres
			{
				firstNameChar = fileReg[0] & 127; //coloca em "0" o bit 7 do primeiro caracter do nome
				
				fileName[0] = firstNameChar;
				int nameCnt = 1;

				while( (nameCnt < 40) && (fileReg[nameCnt] != 0) ) //copia o nome para uma string !!!!cuidado!!!!
				{
					fileName[nameCnt] = fileReg[nameCnt];
					nameCnt ++;
				}
				fileName[nameCnt] = '\0';
				...
				if(strcmp(name,fileName) == 0)
				{
					...
					arrayCpy(openedFile->name, &fileReg[0], 40);
...
char *t2fs_identify(void)
{
	...
	return "Henrique Placido - 209822	Roberto Oppermann Cordoni - 192670";
}
